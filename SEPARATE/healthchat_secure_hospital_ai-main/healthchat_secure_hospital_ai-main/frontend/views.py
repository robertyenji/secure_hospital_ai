...
# MCP server endpoint
MCP_URL = os.environ.get("MCP_SERVER_URL", "https://healthchat-mcp-server.fly.dev/mcp/")
...
def seed_data_run(request):
    try:
        ...
        admissions = create_admissions(patients, staff_pool)
        appointments = create_appointments(patients, staff_pool)
        create_records(patients, staff_pool, appointments)
        create_shifts(staff_pool)
        create_audits(staff_pool)
